...
- 改 `client/` → 必须 `npm run build` 并跑 `test/client-bundle.test.ts` 与 `test/settings-seam.test.ts`，确认注册的槽是 `plugins.bundle.config`、key 与 `package.json` 的 `name` 逐字一致，且 `ctx.configForms.get()` 的实参是 `cordis.patch.yml` 的行 id（**两个 id 今天同串但不是一回事**）；浏览器读的是 **profile 里那份** `lib/client.js`，要随新版本装进 profile 才生效。
