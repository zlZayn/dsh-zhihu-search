...
- 只为一个类型引入新的 `peerDependencies` 不划算：客户端装配包（如 `dsh-api-remotes`）用**结构类型**在本目录就地收窄，写法见 `index.tsx` 的 `ClientRemoteFace`。**槽声明是例外** —— 槽名由提供方合并进共享的 `SlotMap`，结构类型复制不出来，只能 `import type {} from '<包>/client'` 并保留那条边：现在它是 `dsh-client-ui-plugin-manager`（`plugins.row.config`，顺便从同一入口取 `ConfigPageForm` 类型）。**卡片刻意不欠 `dsh-client-ui-settings` 任何类型边** —— 它只经槽位 props 拿表单。
...
- **错误色用 `--dsw-alias-state-error-primary`**：官方 `PluginConfigForm.module.css` 写的 `--dsw-alias-label-error` 在 `ui-theme` 里**从未定义**（实测 0 处），照抄原生会静默不着色 —— 抄官方 CSS 时要顺带核令牌是否存在。
- 注册必须走 `ctx.slots.inject(name, () => ctx.slots.register(...))`，禁止模块级副作用。
- 配置槽的能力探测只**新增提示路径**：槽名 / key / 注册时机一字不改，不查版本号，不因探测失败改动任何既有功能；提示英文、WARN 前缀、落客户端控制台，且**槽迟到必须撤销**（补一条 INFO）。改它要跑产物测试的「配置槽能力探测」一组。
  **它盯的是「卡片拿不到 `form`」这一个真实故障**（2026-09-22 重新定义）：更早的宿主、被当普通 entry 挂载、不是 bundle 行 —— 都归它。接缝换代后旧槽 `plugins.bundle.config` **还在**（只是渲染它时不传 `form`），所以「槽在不在」不再是分水岭，「能不能拿到这一行的表单」才是：**探测必须跟着卡片改槽**，否则它会在新宿主上报 available 而卡片其实在另一个槽 —— **说谎的探测比没有探测更坏**。
