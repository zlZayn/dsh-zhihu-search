 ...
 * 参数白名单。
 *
 * 与站内搜索同理：宿主参数 schema 是开放的（DSL 不接受 `additionalProperties: false`），
...
export function createZhihuGlobalSearchTool(deps: ToolDeps): ToolDefinition {
  return defineTool({
    ...
    description:
      ...
      // 不重复 count / site 的取值（那两处归参数说明，schema 每次都随描述进上下文）；
      // 结果形态槽只写模型拿得到什么、拿不到什么。
      '结果含标题、链接、摘要、作者与时间，知乎来源另带点赞数与评论数，不含图片；没有翻页参数。',
