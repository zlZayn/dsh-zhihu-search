...
- **全网搜索没有翻页参数**（2026-09-13 实测）：把响应里的 `SearchHashId` 分别以 `SearchHashId` / `HashId` / `Cursor` / `Offset` / `Page` / `Start` 传回，结果指纹逐一不变。该端点**静默忽略未知参数**——连非法的 `SortBy` 都回 `code=0`——所以判断「参数是否生效」不能看错误码，只能比结果指纹。
- **`Count` 的服务端硬上限是 20**（实测传 30、50 都只回 20 条；传 0 回落到 10 条）。本地 `MAX_COUNT` 与之对齐，模型传更大值只会被本地截断。
