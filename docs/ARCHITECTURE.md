...
- `zhihu_search` 的 `HasMore` **恒为 false**，不可当作翻页依据；全网搜索的它会为 `true`，含义是「还有匹配未返回」。工具不暴露翻页参数，因此它只在渲染层作为「结果被截断」的提示交给模型，让它收窄关键词或补过滤条件——UI 能看到 `truncated` 却动不了，模型能行动却看不到，这个信号必须落在模型侧。
- ⚠ **未验证**：全网搜索到底有没有取下一批的参数。响应里的 `SearchHashId` 是请求标识而非游标（见 [src/types.ts](../src/types.ts) 的注释），另有一个覆盖全部 19 个端点的第三方项目独立记下「返回 `HasMore`，但无 cursor」。两处都不是实测。**在实测出参数之前，`HasMore` 只能读作「结果被截断」，不能读作「存在下一页」。**
