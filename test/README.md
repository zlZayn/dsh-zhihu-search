...
- [client-bundle.test.ts](client-bundle.test.ts)：浏览器半体的**产物契约**（信封 id、导出面、inject 声明**且不含已删服务**、注册进 `plugins.row.config` 的 key = `<包名>#<行 id>`（两半从声明文件读出）、page 座位把宿主的 `form` 原样透传、`form` 缺席时不抛错、summary 交出行描述，以及**配置槽能力探测**的三条：槽按时到达不发声 / 槽缺席时恰一条英文 WARN（且提示里点名的槽就是实际注册的那个）/ 槽迟到时补一条 INFO 撤销），外加**按真实 Cordis 语义装配**的一组 —— 真实 `Context`、服务由**兄弟** fiber 提供、注入声明取产物自己导出的 `inject`，配三条反向控制。这一组是唯一能看见 inject 门禁的地方：普通替身传的是普通对象，Cordis 的属性代理根本没参与（v1.6.0 卡片加载失败正是这样漏出去的 → [复盘](../docs/postmortem/2026-09-15-client-inject-remote-missing.md)）。
- [settings-seam.test.ts](settings-seam.test.ts)：**设置接缝不变量**（设置接缝迁移的固化）。五条：客户端 `inject` 不含任何已删服务名（黑名单，含宿主半体与脚本）/ 卡片 key 的两半与 `package.json` 和 `cordis.patch.yml` 逐字相等、槽名是 `plugins.row.config` / schema 的 volatile 字段集合**恰好**是 `{accessSecretRef, disableNativeWebSearch}`（密钥绝不 volatile）/ `loader/volatile-update` 的监听与它的 `import type` 都在位、页面策略走 `settings.configure`。这一组全部从 `src/` 与仓内声明文件读，**不 import 产物**。
