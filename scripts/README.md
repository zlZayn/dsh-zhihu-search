...
- `plugin-metadata.mjs`：**展示元数据守卫**。读包根 [locale/](../locale/) 下的语言文件与 [package.json](../package.json)，断言「宿主要读得到插件名与描述」所需的三件事：`locale/<lang>.json` 被 `exports` 暴露（`./locale/*.json`）、被 `files` 收录、且 `meta.title` / `meta.description` 是非空字符串。
...
`check-release.mjs`：**发布态守卫**。断言五条发布态不变量（`dsh.bundle.patch` 在、`private` 未设、`engines.dsh` 已声明、`files` 含 `cordis.patch.yml`、LICENSE 在）**外加展示元数据那三条**，被 `npm run check:release` 调用、[release.yml](../.github/workflows/release.yml) 发布前跑。
...
- **改 `package.json` 的 `exports` / `files` / `description`，或改 [locale/](../locale/) 下的语言文件 → 三处必须同批走**：① 包根 `locale/*.json` 两份（文案真源）；② `package.json`（`files` 收录 `locale/*.json`、`exports` 暴露 `./locale/*.json` 与 `./package.json`、`description` 与 `locale/en.json` 的 `meta.description` 同内容）；③ 回归 `npm run check:release` 与 `test/plugin-metadata.test.ts`（读同一份判定，含反向控制）。改了 `meta.title` 还会挪动插件页上的显示名 → 顺带走一次 [README.md](../README.md) / [README_en.md](../README_en.md) 的「配置」节与 [assets/AGENTS.md](../assets/AGENTS.md) 的判废项。
